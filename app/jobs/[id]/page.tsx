  ...
  const handleShare = async () => {
    ...
    try {
      if (navigator.share) {
        ...
        setFeedback('Content shared successfully!')
      } else if (navigator.clipboard) {
        await navigator.clipboard.writeText(shareData.url)
        setFeedback('Link copied to clipboard!')
      } else {
        setFeedback('Share not supported on this browser.')
      }
    } catch (error) {
      setFeedback('Error sharing content.')
          ...
          <div className='flex flex-col md:flex-row gap-4 items-start'>
            ...
            <div className='flex flex-col gap-2 md:flex-row md:items-center'>
              <button
                type='button'
                onClick={handleShare}
                className='rounded-full border border-gray-500 p-2 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-red'
                aria-label='Share'
              >
              ...
              {feedback && (
                <div className='absolute -top-8 left-1/2 transform -translate-x-1/2 bg-black text-white text-xs px-2 py-1 rounded-md shadow-lg pointer-events-none select-none'>
                  {feedback}
                </div>
              )}
